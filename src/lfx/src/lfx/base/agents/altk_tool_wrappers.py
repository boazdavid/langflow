...
class ProtectedTool(BaseTool):
    ...
    def __init__(
        self, wrapped_tool: BaseTool, toolguard_component=None, conversation_context=None,
            tools=None, guard_path=None, **kwargs
    ):
        super().__init__(
            name=wrapped_tool.name,
            description=wrapped_tool.description,
        ...
        logger.info(f"🔒️ToolGuard processor initialized for {self.name}. tools = {tools}")
    ...
    def _validate_and_run(self, *args, **kwargs) -> str:
        ...
        try:
            from lfx.base.agents.toolguard import toolguard_validation

            # toolGuard invocation point
            result = toolguard_validation(
...
class PreToolGuardWrapper(BaseToolWrapper):
    ...
    def wrap_tool(self, tool: BaseTool, **kwargs) -> BaseTool:
        ...
        if isinstance(tool, ProtectedTool):
            ...
            if "conversation_context" in kwargs:
                tool.update_context(kwargs["conversation_context"])
            logger.debug(f"🔒️Updated existing {tool.name} with {len(self.tools)} tool")
            return tool
